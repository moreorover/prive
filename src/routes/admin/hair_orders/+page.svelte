<script lang="ts">
	import HairOrderCreateFormDialog from '$lib/components/HairOrderCreateFormDialog.svelte';
...
<Card.Root>
	<Card.Header data-testid="orders-header">
		<div class="flex items-center justify-between space-y-2">
			<div>
				<Card.Title data-testid="orders-title">Hair Orders</Card.Title>
				<Card.Description>List of Hair Orders</Card.Description>
			</div>
			<div class="flex items-center space-x-2">
				...
				{#if data.roles.includes('Admin')}
					<HairOrderCreateFormDialog form={data.createOrderForm} />
				{/if}
			</div>
		</div>
	</Card.Header>
	...
	<Table.Root>
		<Table.Caption>A list of your recent hair orders.</Table.Caption>
		<Table.Header>
			<Table.Row>
			...
			{#each $searchStore.filtered as invoice, i (i)}
				<Table.Row>
					...
					<Table.Cell class="text-right"
						><Button
							variant="secondary"
							href={`/admin/hair_orders/${invoice.id}`}
